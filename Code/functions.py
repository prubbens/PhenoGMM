...
'''
def concat_df(df_meta, columns_rep, n_cells, path_data): 
    df = pd.DataFrame()
    for index in df_meta.index:
        for header in columns_rep: 
            df_rep = pd.read_csv(path_data + df_meta.loc[index,header], index_col=0, header=0)
...
'''
def concat_rep(path_data, list_filenames, idx_comm, frac, n_cells, n_rep): 
    df = pd.DataFrame()
    for i in np.arange(0,n_rep): 
        df_rep = pd.read_csv(path_data + list_filenames[n_rep*idx_comm+i], index_col=0, header=0)
...
def get_fcfp_binning_grid(path_data, list_filenames, df_comp, features, grids, n_cells, n_rep, norm_bool=False, transform_bool=True):
    ...
    for i in np.arange(df_comp.shape[0]): 
        ...
        for grid, comb_feat in zip(grids,bivariate_combs): 
            hist, grid_ = np.histogramdd(df_trans.loc[:,comb_feat].values, bins=grid, normed=norm_bool) 
            hist = np.divide(hist, np.float(df_trans.shape[0]))
...
def get_fcfp_gmm(path_data, list_filenames, df_comp, features, gmm_fitted, n_cells, n_rep, n_mixtures=100,transform_bool=True): 
    ...
    for i in np.arange(df_comp.shape[0]): 
        ...
        if transform_bool==True: 
            ...
            df_trans = df_comm
        preds = gmm_fitted.predict(df_trans)
